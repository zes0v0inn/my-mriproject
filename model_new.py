...
from mamba_transunet3d import MambaTransUNet3D, UncertaintyWeightedLoss
...
class SegWithClassifier(nn.Module):
    ...
    def __init__(
        ...
        num_classes: int = 1,
        dropout: float = 0.3,
        seg_only: bool = False,     # True → forward() 只返回 seg tensor，不返回 cls
    ):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.seg_only:
            return seg_out                  # tensor (B, C, D, H, W)
        return seg_out, cls_out             # tuple  (tensor, tensor)
...
def _build_backbone(
    ...
    m2f_num_layers: Optional[int] = None,
    m2f_num_heads: int = 8,
    ...
    if name == "basic_unet":
        ...
        return TransUNet3D(
            ...
            m2f_hidden_dim=m2f_hidden_dim,
            m2f_num_layers=m2f_num_layers,
            m2f_num_heads=m2f_num_heads,
            m2f_mlp_ratio=m2f_mlp_ratio,
            m2f_dropout=m2f_dropout,
        )

    elif name == "mamba_transunet3d":
        return MambaTransUNet3D(
            in_channels=in_channels,
            out_channels=out_channels,
            features=features,
            dropout=dropout,
        ...
        raise ValueError(
            ...
            f"Available: basic_unet, monai_unet, attention_unet, unetr, "
            f"swin_unetr, transunet3d, mamba_transunet3d"
        )
...
def build_model(
    ...
    seg_only: bool = False,         # with_classifier=True 时，是否只返回 seg tensor
    # ── TransUNet3D Mask2Former 选项 ────────────────────────────────
    use_mask2former: bool = False,
    m2f_hidden_dim: int = 256,
    m2f_num_layers: Optional[int] = None,
    m2f_num_heads: int = 8,
